...
fun getGuidesByCategory(category: String): List<GuideItem> {
    val allGuides = listOf(
        GuideItem("1", "CPR for Adults", "Critical life-saving technique for cardiac arrest", "medical", 1, 5,
            listOf("Check responsiveness", "Call 911", "Position hands correctly", "Push hard and fast", "Give rescue breaths if trained")),
...
fun getEmergencyContacts() = listOf(
    EmergencyContact("911 Emergency", "911", "Police, Fire, Medical Emergency", Icons.Default.Emergency, true),
    EmergencyContact("Police", "911", "Law enforcement emergency", Icons.Default.LocalPolice, true),
    EmergencyContact("Fire Department", "911", "Fire emergency services", Icons.Default.LocalFireDepartment, true),
    EmergencyContact("Medical Emergency", "911", "Ambulance and medical services", Icons.Default.LocalHospital, true),
    EmergencyContact("Poison Control", "1-800-222-1222", "24/7 poison emergency helpline", Icons.Default.MedicalServices, false)
)
